...
class TeamDataService {
	...
	async getTeamsByLeagueId(leagueId) {
		const response = await http.get(`teams/byLeague/${leagueId}`);
		return response.data;
	}
					...
					.then(users => {
						...
						team.leagues.forEach(teamLeague => {
							...
							leagues.push([
								...
								{ className: "", text: teamLeague.league.season }, {
									className: "",
									text: `${teamLeague.league.startDate} - ${teamLeague.league.endDate}`
								},
								{ className: "", text: teamLeague.league.stage }, {
									className: "",
									text: teamLeague.originalName
								},
								{
									className: "",
									text: teamLeague.placement + utilityService.getPlacementSuffix(teamLeague.placement)
								}, { className: "", text: teamLeague.winLoss }
							])
						})
						team = {
							...team,
